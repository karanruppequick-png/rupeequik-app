...
export default function PersonalLoanPage() {
  ...
  useEffect(() => {
    async function fetchOffers() {
      try {
        const res = await fetch('/api/offers?category=personal-loan');
        const data = await res.json();
        setOffers(data);
        
        // Initialize calculators
        const calcs: Record<string, boolean> = {};
        data.forEach((o: Offer) => {
          calcs[o.title] = true;
        });
  ...
  const handleApply = () => {
    ...
    setError('');
    router.push(`/apply?category=personal-loan&phone=${mobileNumber}&pan=${pan.toUpperCase()}`);
  };
      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10 text-center lg:text-left">
            <h2 className="text-3xl lg:text-4xl font-bold text-[#1C295E]">Find Best Personal Loan Offers</h2>
          </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-6 p-6 lg:p-8">
                    ...
                    <div className="flex-1 space-y-8">
                      ...
                      <div className="grid sm:grid-cols-2 gap-y-3 gap-x-6 pt-4 border-t border-slate-100">
                        {(bank.benefits || '').split(',').map((feature, i) => (
                          feature.trim() && (
          ...
          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_rgb(0,0,0,0.03)] overflow-hidden">
            <div className="flex flex-col lg:flex-row">
               ...
               <div className="lg:w-[60%] p-8 lg:p-12">
                 <h2 className="text-[28px] font-bold text-[#1C295E] mb-8">Calculate Your Personal Loan EMI</h2>
